...
path =  "Resources/apple_letters.jpg"

img = cv2.imread(path)

gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
gray = cv2.GaussianBlur(gray, (5, 5), 0) # 이미지 블러 처리

cv2.imshow('blur', gray)
cv2.waitKey(0)
mser = cv2.MSER_create() # mser
...
for i, c1 in enumerate(hulls):
    ...
    for j, c2 in enumerate(hulls):
        ...
        # 큰 사각형 안에 작은 사각형이 있으면 제거리스트에 저장
...
text_only = cv2.bitwise_and(img, img, mask=mask) # bitwise_and
